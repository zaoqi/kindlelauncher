...
//import com.mobileread.ixtab.kindlelauncher.util.JARClassLoader;
 ...
 //   private static final LogMessage MENU_ATTRIBUTES = new LogMessage("MenuLoad", new String[]{"name", "value"});
  ...
  //          KindleLauncher.LOG.error("Error parsing extension menu. Cannot parse JSON menu. " + e.getMessage());
    ...
    //	return;
		 ...
		 * 
		 * JSONMenu menu; boolean isJava = false; boolean isDynamic = false; //
		 * String jarFile = null; int i = parser.getAttributeCount(); //
		 * KindleLauncher.LOG.debug("Attributes: " +
		 * parser.getAttributeCount()); while (i-- > 0) { String name =
		 * parser.getAttributeName(i); String value =
		 * parser.getAttributeValue(i); if (name.equals("type")) isJava =
		 * value.equals("java"); else if (name.equals("dynamic")) isDynamic =
		 * value.equals("true"); else if (name.equals("jar")) { // jarFile =
		 * value; } // KindleLauncher.LOG.debug(MENU_ATTRIBUTES, new
		 * String[]{name, value}, ""); }
		 * 
		 * String text = parser.nextText();
		 * 
		 * if (isJava) { // JARClassLoader classLoader =
		 * JARClassLoader.getInstance(); // classLoader.addJar(new File(extDir,
		 * jarFile)); // Class cls = classLoader.loadClass(text); // menu =
		 * (JSONMenu) cls.newInstance(); //
		 * KindleLauncher.LOG.debug("Loaded class: " + text); } else { File
		 * jsonFile = new File(extDir, text); JSONMenu jsonMenu = new
		 * JSONMenu(jsonFile); jsonMenu.setDynamic(isDynamic);
		 * jsonMenu.parseJSONMenu(); menu = jsonMenu; extObj.setMenu(menu); //
		 * KindleLauncher.LOG.debug("Loaded menu: " +
